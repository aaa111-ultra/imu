...
if __name__ == "__main__":
    ...
    for epoch_i in range(epoch, conf.train.max_epoches):
        print(f"当前轮次: {epoch_i+1}/{conf.train.max_epoches}")
        ...
        loss_file = os.path.join(conf.general.exp_dir, "loss_history.csv")
        if epoch_i == 0 and not os.path.exists(loss_file):
            with open(loss_file, "w") as f:
                f.write("epoch,train_loss,test_loss\n")
        with open(loss_file, "a") as f:
            f.write(f"{epoch_i},{train_loss['loss']},{test_loss['loss']}\n")
